...
public class BoardController {
	...
	@ApiOperation(value = "게시글 댓글(QnA)보기", notes = "글번호에 해당하는 게시글의 댓글(QnA)들의 정보를 반환한다.", response = MemoDto.class)
	@GetMapping("/memo/{articleno}")
	public ResponseEntity<List<MemoDto>> listMemo(@PathVariable("articleno") @ApiParam(value = "얻어올 글의 글번호.", required = true) int articleno) throws Exception {
		logger.info("listMemo - 호출 : " + articleno);
		return new ResponseEntity<List<MemoDto>>(boardService.listMemo(articleno), HttpStatus.OK);
	}
	
	@ApiOperation(value = "게시판 댓글(QnA)작성", notes = "새로운 댓글(QnA) 정보를 입력한다. 그리고 DB입력 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@PostMapping("/memo/{articleno}")
	public ResponseEntity<String> writeMemo(@RequestBody @ApiParam(value = "댓글 정보.", required = true) MemoDto memoDto) throws Exception {
		logger.info("writeMemo - 호출 : ");
		logger.info(memoDto.getComment());
		logger.info(memoDto.getUserid());
		logger.info(""+memoDto.getArticleno());
		if (boardService.writeMemo(memoDto)) {
	...
	@DeleteMapping("/memo/delete/{memono}")
	public ResponseEntity<String> deleteMemo(@PathVariable("memono") @ApiParam(value = "살제할 댓글번호.", required = true) int memono) throws Exception {
		logger.info("deleteMemono - 호출");
		if (boardService.deleteMemo(memono)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
		return new ResponseEntity<String>(FAIL, HttpStatus.NO_CONTENT);
	}
